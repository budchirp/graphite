...
Value *IntegerExpression::codegen(const shared_ptr<CodegenContext> &context) const {
  return ConstantInt::get(llvm::Type::getInt32Ty(*context->llvm_context), value,
                          true);
}

void IntegerExpression::analyze(
...
string IntegerExpression::to_string_tree() const {
  return "IntegerExpression(type: " + type->to_string_tree() + ", value: '" + ::to_string(value) + "')";
}

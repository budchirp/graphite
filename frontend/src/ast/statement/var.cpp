  ...
  if (expression) {
    ...
    if (variable_type) {
      if (!TypeHelper::compare(variable_type->get_type(),
        ...
        Logger::type_error("Type mismatch", variable_type->get_position(),
                           variable_type->get_type(), expression->get_type());
        return;
      }
    }
  }
...
void VarStatement::resolve_types(const shared_ptr<ProgramContext> &context) {
  ...
  scope->add_variable(
      ...
      make_shared<VariableSymbol>(
          name->value, SymbolLinkageType::Value::Internal, visibility, type,
          is_global, is_mutable, is_initialized));
}

string VarStatement::to_string() const {
  ostringstream oss;
  oss << "var " << name->to_string()
